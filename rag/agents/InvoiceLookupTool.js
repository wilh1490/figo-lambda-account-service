...
export const schema = {
  name: "InvoiceLookupTool",
  description: `Get details about invoices .`,
  input_schema: {
    type: "object",
    properties: {},
    required: [],
...
export async function handler({
  ...
  const context = await queryOpenSearch({ index: "invoice", filter, must });

  if (!context && intents?.length <= 1) {
    let text =
      "No information available about this invoice yet. Open the app to record an invoice.";
