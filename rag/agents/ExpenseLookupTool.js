...
export const schema = {
  name: "ExpenseLookupTool",
  description: `Get details about expenses .`,
  input_schema: {
    type: "object",
    properties: {},
    required: [],
...
export async function handler({
  ...
  const context = await queryOpenSearch({ index: "expense", filter, must });

  if (!context && intents?.length <= 1) {
    await sendFigoApp({
      wa_id,
      message: "No information about your expenses yet.",
    });
    return;
  }
